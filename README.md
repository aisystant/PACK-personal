...
1. **SPF (Second Principles Framework)** — фреймворк вторых принципов
   - Задаёт форму (структуру) и процесс (производство) для доменных знаний
   - Основан на FPF (First Principles Framework)

2. **Pack «Характеристики и состояния созидателя»** — один конкретный pack, реализованный с помощью SPF
   - Формализованное знание (не информация!) о характеристиках и состояниях созидателя
...
**Кратко**:
- `FPF` = фреймворк первых принципов (мета-онтология + язык различений)
- `SPF` = фреймворк вторых принципов (форма + процесс, основан на FPF)
- `Pack` = формализованное знание конкретной области (основано на SPF)
...
|---------|-----|-----------------|
| **FPF** | Фреймворк первых принципов: мета-онтология + язык различений | Универсален — применим к любой области |
| **SPF** | Фреймворк вторых принципов: форма + процесс (основан на FPF) | Универсален по форме и процессу |
| **Pack** | Формализованное знание (не информация!) | Специфичен для своего домена |
| **Downstream** | Производные представления Pack | Свободная форма и процесс |
