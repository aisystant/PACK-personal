...
| **Каскад планирования** | Иерархия планов от года к дню | ≠ Gantt, ≠ todo-list |
...
| **Правило 3 приоритетов** | Лимит активных фокусов (≤3) для предотвращения перегрузки | ≠ Eisenhower, ≠ ABC-анализ |
...
| **Time-blocking** | Метод планирования, при котором задачи распределяются по фиксированным блокам времени в календаре | ≠ to-do list, ≠ расписание по умолчанию |
...
## Носитель готовности и несовместимые будущие

Для каждой работы план называет не только ожидаемый рабочий продукт, но и **носитель готовности**: конкретный файл, запись, событие, состояние системы или человек, по которому принимающая сторона увидит результат. Формулировка «разобраться», «улучшить» или «подготовить» без носителя готовности не попадает в календарь.

Один операционный план не должен одновременно обещать несовместимые будущие. Если решение ещё не принято, варианты остаются прогнозами или сценариями. После выбора один вариант становится планом, остальные фиксируются как отвергнутые или резервные условия пересмотра. Это предотвращает ложную точность, когда календарь содержит взаимоисключающие работы как будто все они обязательны.

Проверка перед размещением:

- кто принимает результат;
- на каком носителе видна готовность;
- какое одно будущее выбрано;
- какое событие разрешает пересмотреть выбор.
