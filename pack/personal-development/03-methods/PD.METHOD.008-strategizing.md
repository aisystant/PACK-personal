...
|-----------|----------|--------|
| **Проблема** | Базовая проблемная область, к которой относится разрыв (безопасность, признание, социализация, познание) | Безопасность |
...
**Реестр рабочих продуктов (все учтены в стадиях):**
- **Непрерывные:** мимолётные заметки, контексты РП, коммиты, кэш активных РП
- **Долгоживущие:** список неудовлетворённостей, документ стратегии, список проектов, повестка сессии, реестр РП
- **Периодические:** план недели, план дня, записи сессий
- **Архивы:** архив планов, архив стратегии, архив контекстов РП
- **Знания наружу:** Pack, CLAUDE.md, память, документация DS

| # | Стадия | Частота | Вход | Действие | Выход |
|---|--------|---------|------|----------|-------|
| 1 | **Потребление сигналов** | непрерывно | Жизнь, работа, чтение, разговоры; коммуникация с ИИ, рефлексия; артефакты и коммиты дня | Ловить сигнал в моменте; записывать в мимолётные заметки (через бот или вручную); не классифицировать сразу | Мимолётные заметки |
| 2 | **Сессия стратегирования** | еженедельно (пн) | Повестка сессии; список неудовлетворённостей; документ стратегии (результаты месяца); список проектов; реестр РП; кэш активных РП; план прошлой недели (итоги); мимолётные заметки; коммиты прошлой недели | Ревью неудовлетворённостей; ревью прошлой недели + мультипликатор; стоп-лист по активным РП; фокус месяца; Note-Review по категориям; приоритизация (ТОС + отбор РП через [PD.METHOD.017](PD.METHOD.017-wp-selection.md)); инвариант «РП ≥3ч → к какому результату месяца и проекту?»; проверка связи результата с проектом; запрос недели; фиксация | План недели (новый); список неудовлетворённостей (обновлённый); документ стратегии (скорректированный фокус); кэш активных РП (полная перезапись); запись сессии |
| 3 | **Открытие дня + Inbox Triage** | ежедневно (утро) | План недели; коммиты вчерашнего дня; новые мимолётные заметки; кэш активных РП; календарь встреч | Inbox Triage (capture-кандидаты ≥14 дней, сверка frontmatter с кэшем); Note-Review новых заметок; выбор РП на день с бюджетом; формирование плана дня | План дня; кэш активных РП (статусы); мимолётные заметки (пометки обработки) |
| 4 | **Исполнение** | в течение дня | План дня; новые сигналы в работе | Работа по РП (WP Gate при ≥15 мин); capture-loop «сейчас или отложить?» → мимолётные заметки; Capture-to-Pack на рубежах (правило → CLAUDE.md, доменное → Pack, реализационное → документация DS, урок → память); ArchGate при архитектурных решениях; коммиты в целевые репо | Коммиты; артефакты (код, документы, черновики); обновления Pack / CLAUDE.md / памяти / документации DS; новые мимолётные заметки; контексты РП |
| 5 | **Закрытие дня** | ежедневно (вечер) | План дня; коммиты дня; учёт времени (факты дня) | Ревью закрытых РП; похвала (что удалось); KE-маршрутизация «что узнали»; обновление статусов в кэше РП; 3 варианта плана на завтра | Закрытый план дня (с отчётом); кэш активных РП (обновлённый); KE-captures в Pack / CLAUDE.md / память |
| 6 | **Закрытие недели** | еженедельно (вс/пн до сессии) | План недели; коммиты недели (все репо); учёт времени; выполненные РП | Сбор статистики (коммиты/репо/часы); мультипликатор (бюджет vs факт); ревью закрытых/перенесённых РП; архивация done-РП (кэш → реестр); перенос плана в архив (после следующей сессии) | План недели с секцией «Итоги»; кэш активных РП (очищен от done); реестр РП (обновлён); архив планов; архив контекстов РП (done) |
| 7 | **Закрытие месяца** | первый пн месяца (надстройка к сессии) | Планы недель прошлого месяца; документ стратегии (результаты); список проектов; список неудовлетворённостей | Ретро по результатам месяца (done / перенесено / провалено); состояние стратегической позиции (Развитие / Хаос / Потолок / Пивот); ревизия проектов (активные / заморожены / новые); синхронизация неудовлетворённости ↔ результаты; архив старого фокуса; новый фокус месяца | Документ стратегии (новый фокус); список проектов (ревизован); список неудовлетворённостей (синхронизован); архив стратегии (снимок); запись сессии (Month Close) |

**Инвариант цепочки:** carry-over каждого контура (день, неделя, месяц) переносится во вход следующей итерации того же уровня. Обратно: отчёты агрегируют факты (стадия 5 → стадия 6 → стадия 7). Все рабочие продукты из реестра выше участвуют хотя бы в одной стадии как Вход или Выход — если РП не участвует ни в одной, он не нужен.
