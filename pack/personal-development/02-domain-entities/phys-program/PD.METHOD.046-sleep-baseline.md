...
| Force | Tension |
|-------|---------|
| Точность биометрии ↔ доступность без wearable | Метод предполагает данные Pillow за 14 дней (§«Вход»), но при отсутствии устройства допускается менее точная ручная оценка «во сколько лёг/встал» (§«Wearable dependency») — порог входа конкурирует с точностью сигнала |
| Стабильность baseline ↔ чувствительность к смене ритма | Baseline пересчитывается раз в месяц (§«Calibration period»), что даёт устойчивый порог, но смена ритма жизни (переезд, новая работа) требует внепланового пересчёта — иначе устойчивый порог перестаёт отражать реальность |
| Порог saneness ↔ соблазн оптимума | Метод явно фиксирует «baseline, не оптимум» (§«Метрика baseline»), но восьмичасовой сон с REM-фазами как ориентир конкурирует со скромным порогом «носитель работает достаточно», который метод удерживает |
...
|------|--------------------------|
| Удобство ручного ввода перевешивает точность | При отсутствии Pillow оценку «во сколько лёг/встал» проще вносить вечером, хотя метод требует утреннюю оценку в течение 2ч после пробуждения (§«Latency sensitivity») — внимание смещается от точной привязки ко времени к простоте записи |
| Просадка читается как повод для интервенции | Метод (специализация Time Accounting) регистрирует факт, но при 2 ночах подряд ниже baseline внимание практика съезжает к попытке немедленно «исправить» сон, минуя различение measurement/intervention |
