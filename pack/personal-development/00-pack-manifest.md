...
| PD.FORM.089 | Learner Rcs | FORM | Role Characterisation Space (RCS) Ученика: 7 диагностических срезов узлов Созидателя (Человек+IWE). Каждый срез — квартет (характеристика, показатель, baseline, потенциал) по FORM.088 §2. W→Мыслительное мастерство; M1→Организм; M2→Управление IWE; M3→Проф.мастерство(вне ЛР); M4→Мыслительное мастерство; IT→IWE-канал; A→Созидатель. Ступень = min по срезам мастерства (bottleneck). Формализм: FPF A.19 U.CharacteristicSpace | draft |
...
> *Auto-generated by `generate-map.py` on 2026-05-15*
