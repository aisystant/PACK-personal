...
| PD.FORM.089 | Learner Rcs | FORM | Role Characterisation Space (RCS) Ученика: болид-онтология (Пилот+Машина), 3 узла ЛР (Мыслительное/Саморазвитие/Управление IWE), 7 bh-характеристик Аттестатора (bh.sys/inv/met/awr/agn/scl/stb) + 13 cp-срезов Диагноста (cp.rhy/wld/skl/iwe/cre/knw/env/int/auto/exo/agt/cult/traj). Ступень = двойной gate: bh-сигнал (Аттестатор, авто) → cp-подтверждение (Диагност, по запросу). Формализм: FPF A.19 U.CharacteristicSpace | draft |
